...
function giftcodeHandleMove(e) {
  ...
  const centerX = giftcodeRect.width / 3;
  const centerY = giftcodeRect.height / 3;
  ...
  const shadowX = -deltaX * 4;
  const shadowY = -deltaY * 4;
...
function giftcodeHandleEnd() {
  cancelAnimationFrame(giftcodeRafId);
  giftcodeCard.style.transition = 'transform 0.8s ease-out, box-shadow 0.8s ease-out';
